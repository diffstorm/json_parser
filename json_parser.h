...
const char *json_error_string(json_error_t error) {
    switch (error) {
        ...
        case JSON_ERROR_INVALID_TOKEN: return "Invalid token";
        case JSON_ERROR_UNEXPECTED_CHAR: return "Unexpected character";
        case JSON_ERROR_MAX_TOKENS: return "Max tokens exceeded";
        case JSON_ERROR_STRING_TOO_LONG: return "String too long";
        case JSON_ERROR_INVALID_ESCAPE: return "Invalid escape sequence";
        case JSON_ERROR_INVALID_UNICODE: return "Invalid Unicode escape";
        case JSON_ERROR_NESTING_DEPTH: return "Nesting depth exceeded";
        case JSON_ERROR_INVALID_NUMBER: return "Invalid number format";
        case JSON_ERROR_TRAILING_CHARS: return "Trailing characters";
        case JSON_ERROR_ALLOCATION_FAILED: return "Memory allocation failed";
        default: return "Unknown error";
    }
}

const json_token_t *json_get_tokens(const json_parser_t *parser, size_t *count) {
    if (count) *count = parser->token_count;
...
static int json_parse_unicode_escape(json_parser_t *parser, char *buf, size_t *idx) {
    if (parser->pos + 4 > parser->length) return -1;
    ...
    if (codepoint <= 0x7F) {
        if (*idx >= parser->max_string - 1) return -1;
        buf[(*idx)++] = codepoint;
    } else if (codepoint <= 0x7FF) {
        if (*idx >= parser->max_string - 2) return -1;
        buf[(*idx)++] = 0xC0 | (codepoint >> 6);
        buf[(*idx)++] = 0x80 | (codepoint & 0x3F);
    } else if (codepoint <= 0xFFFF) {
        if (*idx >= parser->max_string - 3) return -1;
        buf[(*idx)++] = 0xE0 | (codepoint >> 12);
        buf[(*idx)++] = 0x80 | ((codepoint >> 6) & 0x3F);
        buf[(*idx)++] = 0x80 | (codepoint & 0x3F);
    } else {
        if (*idx >= parser->max_string - 4) return -1;
...
static int json_parse_string(json_parser_t *parser) {
    if (json_add_token(parser, JSON_TOKEN_STRING)) return -1;
...
static int json_parse_number(json_parser_t *parser) {
    ...
    if (json_add_token(parser, JSON_TOKEN_NUMBER)) return -1;
    json_token_t *token = &parser->tokens[parser->token_count - 1];
...
static int json_parse_literal(json_parser_t *parser, const char *literal, json_token_type_t type) {
    ...
    if (parser->pos + len > parser->length) return -1;
    
    if (strncmp(parser->json + parser->pos, literal, len)) return -1;
    
    parser->pos += len;
    if (json_add_token(parser, type)) return -1;
    return 0;
}
...
static int json_parse_value(json_parser_t *parser) {
    ...
    switch (c) {
        ...
        case 't': return json_parse_literal(parser, "true", JSON_TOKEN_TRUE);
        case 'f': return json_parse_literal(parser, "false", JSON_TOKEN_FALSE);
        case 'n': return json_parse_literal(parser, "null", JSON_TOKEN_NULL);
        default:
            if (c == '-' || isdigit(c) || c == '.') {
...
static int json_parse_object(json_parser_t *parser) {
    ...
    while (parser->pos < parser->length) {
        ...
        if (json_parse_value(parser)) return -1;
        
        json_skip_whitespace(parser);
        if (parser->json[parser->pos] == '}') continue;
        if (parser->json[parser->pos++] != ',') {
            json_set_error(parser, JSON_ERROR_UNEXPECTED_CHAR);
            return -1;
        }
